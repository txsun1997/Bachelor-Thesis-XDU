...
class Embeddings(nn.Module):
    def __init__(self, we, vocab, embed_dim, freeze):
        ...
        self.cls_embedding = nn.Parameter(torch.Tensor(1, 16, embed_dim))
    ...
    def forward(self, x):
        ...
        embeddings = torch.cat([self.cls_embedding.expand(bsz, -1, -1), embeddings], dim=1)
...
class MLP(nn.Module):
    def __init__(self, d_model, d_ff, n_class, dropout=0.5):
        super(MLP, self).__init__()
        self.w_1 = nn.Linear(d_model, d_ff)
        self.w_2 = nn.Linear(d_ff, n_class)
        self.dropout = nn.Dropout(dropout)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, config, word_embedding):
        ...
        # mlp = MLP(d_model, d_ff, args.n_class)
    ...
    def forward(self, task_id, x, y):
        ...
        cls_mask = torch.ByteTensor([1] * bsz).unsqueeze(1).expand(-1, 16)
